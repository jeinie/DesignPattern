public class BallMachine {
    ...
    final static int SOLD_OUT = 0; // 상태는 변하지 않으므로 final
    ...
    public BallMachine(int count) {
        this.count = count;
        // 알맹이 갯수가 0이 아니면 동전을 누군가가 넣어주길 기다리고 있는 NO_COIN 상태로 전환됨
    ...
    public void insertCoin() {
        if (state == HAS_COIN) {
            System.out.println("동전은 한 개만 넣어주세요."); // 동전이 투입된 경우
        }
        // 동전이 투입된 경우
    ...
    public void dispense() {
        // 나오면 안되는 오류 상황
        if (state == HAS_COIN) {
            System.out.println("볼을 내보낼 수 없습니다."); // 오류
        }
        else if (state == NO_COIN) {
            System.out.println("동전을 넣어 주세요");
        }
